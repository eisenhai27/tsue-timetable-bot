...
import { fmtChanges, fmtWeek, tr, tashkentNow, addDays, mondayOf } from '../src/shared.mjs';
...
async function main() {
  ...
  if (mode === 'changes') {
    ...
    for await (const sub of subscribers('weekly')) {
      const g = await loadGroup(sub.group_id);
      if (!g) continue;
      await send(sub.chat_id, fmtWeek(g, index, monday, sub.lang), await keyboard(sub, g.id));
    }
  } else {
